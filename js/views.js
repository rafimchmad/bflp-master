...
(function(){
  ...
  // ---------------- PDF VIEWER ----------------
  function pdfView(ch,page){ const c=CH.find(x=>x.ch===ch); if(!c)return home(); page=page||1;
    Store.markRead(ch,page); Store.pushRecent({type:'pdf',ch,page,label:`Bab ${ch} hal.${page}`});
    setCrumb([{t:'Beranda',go:{view:'home'}},{t:'Bab '+ch,go:{view:'chapter',ch}},{t:'PDF hal. '+page}]);
    const src=pdf(ch)+'#page='+page+'&zoom=page-width';
    root().innerHTML=`
      <div class="pdf-toolbar">
        <button class="btn sm" id="pv-prev">‹ Prev</button>
        <span>Hal.</span><input id="pv-page" type="number" value="${page}" min="1" max="${c.pages}"><span>/ ${c.pages}</span>
        <button class="btn sm" id="pv-next">Next ›</button>
        <button class="btn sm" id="pv-go">Jump</button>
        <span style="margin-left:auto"></span>
        <button class="btn sm" data-bookmark data-ch="${ch}" data-page="${page}">☆ Bookmark hal ini</button>
        <button class="btn sm" id="pv-note">📝 Catatan</button>
        <button class="btn sm" id="pv-open">↗ Tab baru</button>
      </div>
      <iframe class="pdf-frame" id="pv-frame" src="${src}"></iframe>`;
    const go=p=>{ p=Math.max(1,Math.min(c.pages,+p||1)); $('#pv-frame').src=pdf(ch)+'#page='+p+'&zoom=page-width'; $('#pv-page').value=p; Store.markRead(ch,p);
      crumb().lastChild; setCrumb([{t:'Beranda',go:{view:'home'}},{t:'Bab '+ch,go:{view:'chapter',ch}},{t:'PDF hal. '+p}]); };
    $('#pv-prev').onclick=()=>go((+$('#pv-page').value)-1);
    $('#pv-next').onclick=()=>go((+$('#pv-page').value)+1);
    $('#pv-go').onclick=()=>go($('#pv-page').value);
    $('#pv-page').onkeydown=e=>{if(e.key==='Enter')go(e.target.value);};
    $('#pv-open').onclick=()=>window.open(pdf(ch)+'#page='+$('#pv-page').value,'_blank');
    $('#pv-note').onclick=()=>{ const txt=prompt('Catatan untuk Bab '+ch+' hal.'+$('#pv-page').value); if(txt){Store.addNote({ch,page:+$('#pv-page').value,text:txt,tag:'PDF',color:'#f37021'});toast('Catatan tersimpan');} };
  }
